...
3. Remove the macOS quarantine flag so Gatekeeper will open the unsigned app:
   ```bash
   xattr -dr com.apple.quarantine /Applications/TaskFlow.app
   ```
   Then launch **TaskFlow** normally from Applications or Spotlight.

> The app is ad-hoc signed but not notarized (no Apple Developer account), so without the step above macOS shows *"TaskFlow is damaged and can't be opened"*. The `xattr` command removes the download-quarantine attribute and is only needed once per install.
